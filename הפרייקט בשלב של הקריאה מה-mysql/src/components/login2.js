...
export default function Login(props) {
    ...
    async function getUser(event) {
        ...
        try {
            let data;
            if(cleaner){
                 data = await fetch(`http://localhost:2024/cleaner/${password}`);
                data = await data.json();
                console.log(data);
               
            }
            else{
                 data = await fetch(`http://localhost:2024/user/${password}`);
                data = await data.json();
                console.log(data);
            ...
            if (!data) {
                ...
                //////////////////////////////////////////
                console.log(data[0]);
                await setCurrent(data[0]);
                if(cleaner){
                    navigate(`/${data[0].password}/homeCleaner`);
                }
                else{
                    navigate(`/${data[0].password}/homeUser`);
                }
            }
        }
        catch (err) {
